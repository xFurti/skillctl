...
We follow [Semantic Versioning](https://semver.org/). All workspace packages are kept at the same version (currently **0.3.1** on npm; **0.4.0** unreleased on `main`). Update `CHANGELOG.md` under `[Unreleased]` or the next version section.
